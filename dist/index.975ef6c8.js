...
const searchParams = new URLSearchParams({
    key: "42569157-750b6d768d4a6a912952071b6",
    q: "",
    image_type: "photo",
...
const fetchPhotos = async ()=>{
    ...
    const searchResults = await (0, _axiosDefault.default).get(`https://pixabay.com/api/?${searchParams}`);
...
function renderPhotos(data, append = false) {
    if (data.hits.length <= 0) {
        ...
        const markup = data.hits.map(({ webformatURL , largeImageURL , tags , likes , views , comments , downloads ,  })=>`<div class="photo-card"><a class="gallery__item" href="${largeImageURL}">
  ...
  <div class="info">
    ...
    <p class="info-item">
      <b>Downloads: ${downloads}</b>
        ...
        if (append) gallery.innerHTML += markup;
...
searchQuery.addEventListener("submit", async (event)=>{
    ...
    if (searchPhrase === "") {
        (0, _notiflixDefault.default).Notify.warning("Please enter a search phrase!");
        return;
    }
    try {
        page = 1;
        ...
        renderPhotos(photos);
        if (photos.hits.length === 0) fetchBtn.classList.add("hidden");
        else {
            fetchBtn.classList.remove("hidden");
            const dataHits = photos.totalHits;
            (0, _notiflixDefault.default).Notify.success(`Hooray! We found ${dataHits} images.`);
        }
    } catch (error) {
        (0, _notiflixDefault.default).Notify.failure(`ERROR: ${error}`);
    }
});
...
function loadMorePhotos() {
    ...
    } else fetchBtn.classList.remove("hidden");
}

},{"axios":"jo6P5","notiflix":"fuMq1","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"jo6P5":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
